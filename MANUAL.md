...
- Amplicon normalisation and primer trimming using the Welkers v4.2 BED scheme
- Variant calling using the specified Medaka nanopore model
- Consensus FASTA generation with ambiguous positions masked as N
...
The combined consensus FASTA is analysed with Nextclade using the local mpox dataset. Nextclade assigns each sample to an MPXV clade and Pango lineage, and calculates genome coverage.
...
### Step 6 — Summary report (`R_COMBINE_RESULTS`)
An R script merges all per-run data into a single result table. This step runs after all UShER steps (step 13) so that phylogenetic placement results can be included.

Data merged:
- Read and mapping statistics (from step 3)
- Nextclade clade/lineage assignments and genome coverage (from step 5)
- UShER closest-neighbour results: distance, number of tied neighbours, countries, and date range (from step 13)
...
### Step 7 — Download global MPXV tree (`USHER_DOWNLOAD`)
...
### Step 8 — Alignment to reference (`USHER_ALIGN`)
The combined consensus FASTA is aligned to the static MPXV reference genome (`assets/UShER/NC_063383.fasta`) using `minimap2` with assembly-to-reference parameters (`asm20`).
...
### Step 9 — Multiple sequence alignment (`USHER_MSA`)
...
### Step 10 — VCF generation (`USHER_MAKE_VCF`)
...
### Step 11 — VCF masking (`USHER_MASK_VCF`)
...
### Step 12 — Phylogenetic placement and tree export (`USHER_PLACE`)
Samples are placed into the global MPXV tree using UShER (Ultrafast Sample placement on Existing tRees):

1. **Greedy placement** (`usher`): Samples are placed using a greedy algorithm minimising parsimony score
...
3. **Tree export** (`matUtils extract`): Both greedy and optimised trees are exported in Newick and JSON formats
4. **Context reports**: Per-sample closest-relative reports are generated — one listing the single nearest neighbour, one listing all neighbours within 10 SNPs
...
### Step 13 — Closest-neighbour summary report (`USHER_REPORT`)
...
│
└── usher/
    ├── {RunName}_full_tree_greedy.nwk              # Newick: greedy placement tree
    ├── {RunName}_full_tree_greedy.json             # JSON: greedy tree with global metadata
...
### `{RunName}_full_tree_optimized.json` and `{RunName}_context_tree.json`
JSON tree files compatible with [Taxonium](https://taxonium.org/) for interactive phylogenetic visualisation. The full tree contains all global sequences plus the run samples. The context tree is a pruned subtree retaining the 20 closest global neighbours per local sample — useful for focused visualisation.

---

### `{RunName}_context_tree.nwk` and `{RunName}_full_tree_optimized.nwk`
Newick format trees for use in R (`ggtree`, `ape`) or other phylogenetic tools. The context subtree Newick is most suitable for publication-quality figures.

---

### `{RunName}_closest_neighbor_report.tsv`
Tab-separated summary table produced by the R closest-neighbour script. One row per local sample:
...
| `Countries` | Semicolon-separated list of countries of closest neighbours |
